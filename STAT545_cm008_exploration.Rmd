...
```{r}
if_else(letters == "h", "H", letters) #sample code using letters vector

gapminder %>% 
  mutate(lifeExp = if_else(country == "Canada" & year == 1952, 70, lifeExp)) %>% 
  filter(country == "Canada") #filter for for ease of viewing
```
...
{BJ: Because there are multiple entries (by year) in gdpPercap and pop for each country, so there is no single entry that is meaningful}
...
Can also add a mutate function with a new variable to output more meaningful text
...
gapminder %>% 
  group_by(country) %>% 
  mutate(growth = pop - first(pop)) #or pop[1]
...
gapminder %>% 
  ...
  filter(!is.na(gain)) %>% #filter out NA's
...
```{r}
gapminder %>% 
  mutate(pop_in_Bn = round(pop/1000000, digits = 2))
```
...
gapminder %>% 
  group_by(country) %>% 
  mutate(delta = pop[year == 1972] - pop[year == 1962]) %>% 
  distinct(delta)
