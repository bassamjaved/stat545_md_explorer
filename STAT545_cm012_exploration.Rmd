...
iris
iris %>% 
  mutate(Species = ifelse(Species == "versicolor", "vers", Species)) #returns the integer value assigned to the factor
  # %>% str()
```
...
as.character(iris$Species) #to convert to a character vector
```


- ~~Base R~~ ways of interacting with factors:
...
```{r}
set.seed(10) #this function makes the iterative procedure the same, so this "randomized" sample is the same for everyone who runs it
...
levels(gap_gs$country) #all the country rows are still stored in the factor
as.character(gap_gs$country) #character vector only has Germany and Sweden
```
...
ggplot(gap_small, aes(continent)) + 
  geom_bar() +
  scale_x_discrete(drop = FALSE) #do not drop the unused factor levels
```
...
```{r}
#droplevels() to entire dateframe
droplevels(gap_small) %>% str()

#drop only continent levels using fct_drop()
mutate(gap_small, continent = fct_drop(continent)) %>% str()
```
...
gap_asia_2007$country %>% 
  fct_relevel("Syria", "Sweden") %>% #Sweden is actually an unused level in gap_asia_2007
  levels() %>% 
  head()
...
cont %>% 
  fct_lump() %>% 
  qplot() #by default makes Other less than the next least frequent continent
