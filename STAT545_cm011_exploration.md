...
``` r
(a <- 7:-2) #parentheses will print out result
```
...
``` r
a * 1:3 #multiplies 1,2,3 repeatedly until the end of the index
```
...
Now try a boolean operator. Note that && and || are NOT
vectorized\!
...
``` r
a > 0 && a < 5 #only looks at first entry of left hand side and first entry of right hand side
```
...
``` r
course #overlays names without changing course vector
```
...
``` r
course[[c("section")]] #returns single entry without name
```
...
``` r
identical(list(4), 4) #list of object 4 and 4 itself
```

    ## [1] FALSE

``` r
identical(c(4), 4) #vector with object 4 and 4 itself
```
