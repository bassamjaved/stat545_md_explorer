...
#compare this lesson to cm009 (spread and gather functions for tidying data)
#how do you modify previous output in Rmd file to include dimensions
...
#numerical operators in R can take the form `OPERATOR`
```
...
```{r}
got_chars[[1]] #have a look at what the output is for the first entry
got_alias <- got_chars %>% {
  tibble(name = map_chr(., "name"),
         aliases = map(., "aliases") #can't use map_chr here; output not always length 1
           )
}
got_alias
```
...
got_alias %>% 
  ...
  #need {} b/c pipe will use got_alias first in the function otherwise
  `[[`("Melisandre")
```
...
```{r}
str(got_alias) #there's a NULL entry, so unnest directly won't work
got_alias %>% 
  drop_na() %>% 
  unnest()
```
