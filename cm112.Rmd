...
```{r}
jsonlite::fromJSON("http://omdbapi.com/?t=Jurassic+Park&apikey=9aecef0f")
```
...
site2 <- GET("https://api.github.com/users/hadley/orgs")
str(site2)
names(site2)
class(site2)
```
...
```{r}
site$status_code
#site$content #stores information in bytes
content(site)

site2$status_code
content(site2) #this site has an API
```
