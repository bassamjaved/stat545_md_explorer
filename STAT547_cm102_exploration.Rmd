...
3. BONUS: Write the README's to file using base R's `write(x, file)`:
    ...
    - This code might not work, depending on your working directory and system.
...
abc <- str_split(fox, pattern = " ")[[1]]
(def <- str_c(abc, "ay"))

pigLatinizer <- function(x) {
  first_letter <- str_sub(x, 1, length(x))
  rest_of_the_word <- str_sub(x, 2)
  pig_latin <- str_c(rest_of_the_word, first_letter, "ay")
  return(pig_latin)
}

test_that("The function works.",
          {expect_equal(pigLatinizer("test"), "esttay")
            }
          )
...
```{r}
str_view_all_match(countries, pattern = "(oo)|(cc)")
```